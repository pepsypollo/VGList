<?php
	$juego=$_GET['id'];
	$consulta="SELECT nombre, publicacion, sinopsis, plataforma, imagen FROM juego j,plataforma p WHERE j.id_plat=p.n_plat AND j.id='".$juego."';";
	$get=mysqli_query($con,$consulta);
	$get=mysqli_fetch_array($get,MYSQLI_ASSOC);
?>
<nav aria-label="breadcrumb">
  <ol class="breadcrumb">
    <li class="breadcrumb-item"><a href="/juegos.php?id=<?php echo $juego; ?>">General</a></li>
    <li class="breadcrumb-item"><a href="/juegos.php?id=<?php echo $juego; ?>&analisis">Análisis</a></li>
    <li class="breadcrumb-item"><a href="/juegos.php?id=<?php echo $juego; ?>&recomendacion">Recomendaciones</a></li>
  </ol>
</nav>
...
<?php
	if (isset($_GET['analisis'])) {
		include "juegoanalisis.php";
	}else{
		if (isset($_GET['recomendacion'])) {
			include "juegorecomendacion.php";
		}else{
?>
		...
		<?php
			...
			if(isset($_SESSION['user'])){
				...
				echo "<form action='/juegos.php?id=".$juego."' method='post' accept-charset='utf-8'>
